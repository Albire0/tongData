<template>
	<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="100px" @keyup.enter="submitHandle()">
		...
		<el-form-item label="名称" prop="name" label-width="auto">
			<el-input v-model="dataForm.name" :disabled="!editFlag" placeholder="名称"></el-input>
		</el-form-item>
		<el-form-item label="任务类型" prop="taskType" label-width="auto">
			<fast-select
				v-model="dataForm.taskType"
				:disabled="!editFlag"
				dict-type="quality_config_task_type"
				placeholder="任务类型"
				clearable
			></fast-select>
		</el-form-item>
		<el-form-item v-if="dataForm.taskType == 2" label="cron表达式" prop="cron" label-width="auto">
			<el-input v-model="dataForm.cron" placeholder="cron表达式" :disabled="!editFlag"></el-input>
			<el-button v-if="dataForm.taskType == 2" type="primary" style="margin-top: 1rem" @click="dialogCronVisible = true">配置Cron表达式</el-button>
		...
		<el-form-item label="备注" prop="description" label-width="auto">
			<el-input v-model="dataForm.description" :disabled="!editFlag" type="textarea"></el-input>
		</el-form-item>
		...
		<el-button v-if="standardCodeValues.length > 0 && editFlag" type="warning" size="small" @click="addConfig">添加配置</el-button>
		<div v-for="(item, index) in dataForm.cleanConfig" v-if="standardCodeValues.length > 0" style="margin-top: 10px">
			<span>库中值：</span>
			<el-input v-model="item.oldValue" :disabled="!editFlag" placeholder="库中值" style="width: 200px"></el-input>
			<span>&emsp;&emsp;清洗值：</span>
			<el-input
				v-if="standardCodeValues.length == 0"
				v-model="item.newValue"
				:disabled="!editFlag"
				placeholder="清洗值"
				style="width: 200px"
			></el-input>
			<el-select v-else v-model="item.newValue" :disabled="!editFlag" clearable filterable placeholder="清洗标准值编号">
				<el-option
					v-for="(code, index) in standardCodeValues"
					:key="code.id"
		...
		<el-form-item label-width="auto" style="margin-top: 10px">
			...
			<el-button v-if="!editFlag" :disabled="standardCodeValues.length == 0" type="danger" @click="handleRun()">执行</el-button>
			<el-button v-if="dataForm.status == 0 && !editFlag" :disabled="standardCodeValues.length == 0" type="warning" @click="release()"
				>发布</el-button
			>
			<el-button v-if="dataForm.status == 1 && !editFlag" :disabled="standardCodeValues.length == 0" type="warning" @click="cancelRelease()"
				>取消发布</el-button
			>
		</el-form-item>
	</el-form>
...
import {
	useStandardCleanApi,
	useStandardCleanSubmitApi,
	getCleanConfigApi,
	getStandardColumnTypeApi,
	getStandardCodeApi,
	getMetaStandardRelApi,
	releaseApi,
	cancelReleaseApi,
	handleRunApi
} from '@/api/data-governance/standardClean'
