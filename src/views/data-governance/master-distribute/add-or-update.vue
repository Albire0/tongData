	...
	<el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false">
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="100px" @keyup.enter="submitHandle()">
			...
			<el-form-item label="派发类型" prop="distributeType" label-width="auto">
				<fast-select
					v-model="dataForm.distributeType"
					dict-type="distribute_type"
					placeholder="派发类型"
					clearable
					@change="distributeTypeChange"
				></fast-select>
			</el-form-item>
			<!-- mq或接口 -->
			...
			<el-form-item v-if="dataForm.taskType == '3'" label="cron表达式" prop="cron" label-width="auto">
				<el-input v-model="dataForm.cron" placeholder="cron表达式"></el-input>
				<el-button v-if="dataForm.taskType == '3'" type="primary" style="margin-top: 1rem" @click="dialogCronVisible = true"
					>配置Cron表达式</el-button
				>
			...
			<el-form-item label="描述" prop="description" label-width="auto">
				<el-input v-model="dataForm.description" type="textarea" placeholder="描述"></el-input>
			</el-form-item>
			<!-- 数据库相关参数 -->
			<template v-if="dataForm.distributeType == 1">
				<el-divider
					><span style="font-size: 17px"><b>接入配置</b></span></el-divider
				>
				<el-form-item label="接入数据库" prop="distributeJson.distributeDb.databaseId" label-width="auto">
					<el-select v-model="dataForm.distributeJson.distributeDb.databaseId" clearable filterable placeholder="请选择">
						<el-option v-for="(item, index) in databases" :key="item.id" :label="`[${item.id}]${item.name}`" :value="item.id"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="只创建表" label-width="auto" prop="distributeJson.distributeDb.onlyCreate">
				...
				<el-form-item label="是否同步已存在的表" label-width="auto" prop="distributeJson.distributeDb.syncExist">
					<el-select v-model="dataForm.distributeJson.distributeDb.syncExist">
						<el-option label="是" :value="true"></el-option>
						<el-option label="否" :value="false"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="同步前是否删除目的表" label-width="auto" prop="distributeJson.distributeDb.targetDrop">
					<el-select v-model="dataForm.distributeJson.distributeDb.targetDrop">
						<el-option label="是" :value="true"></el-option>
						<el-option label="否" :value="false"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="是否启用增量变更同步" label-width="auto" prop="distributeJson.distributeDb.changeDataSync">
				...
				<el-form-item label="数据处理批次大小" label-width="auto" prop="distributeJson.distributeDb.fetchSize">
				...
				<el-divider
					><span style="font-size: 17px"><b>映射配置</b></span></el-divider
				>
				<div>
					...
					<el-table
						:data="dataForm.distributeJson.distributeDb.regexTableMapper"
						size="small"
						border
						height="200"
					...
					<el-table
						:data="dataForm.distributeJson.distributeDb.regexColumnMapper"
						size="small"
						border
						height="200"
						style="width: 90%; margin-top: 15px"
			...
			<template v-if="dataForm.distributeType == 2">
				...
				<el-form-item label="数据推送批次大小" label-width="auto" prop="distributeJson.distributeApi.fetchSize">
					<el-select v-model="dataForm.distributeJson.distributeApi.fetchSize">
						<el-option label="5" :value="5"></el-option>
						<el-option label="10" :value="10"></el-option>
						<el-option label="30" :value="30"></el-option>
						<el-option label="50" :value="50"></el-option>
						<el-option label="100" :value="50"></el-option>
					</el-select>
				</el-form-item>
				<el-divider
					><span style="font-size: 17px"><b>参数配置</b></span></el-divider
				>
				<div>
					<el-button type="success" round @click="addUrlHeader()">添加header</el-button>
					<el-table :data="dataForm.distributeJson.distributeApi.headers" size="small" border height="150" style="width: 90%; margin-top: 15px">
						<template #empty>
						...
						<el-table-column label="value" width="320">
							<template #default="scope">
						...
						<el-table-column label="操作" width="220">
							<template #default="scope">
								<el-button size="small" type="danger" @click="deleteUrlHeader(scope.$index)">删除</el-button>
							</template>
						</el-table-column>
					</el-table>
					<br />
					<el-button type="success" round @click="addUrlParam()">添加param</el-button>
					<el-table :data="dataForm.distributeJson.distributeApi.params" size="small" border height="150" style="width: 90%; margin-top: 15px">
						<template #empty>
						...
						<el-table-column label="操作" width="220">
							<template #default="scope">
								<el-button size="small" type="danger" @click="deleteUrlParam(scope.$index)">删除</el-button>
							</template>
						</el-table-column>
					</el-table>
				</div>
			...
			<template v-if="dataForm.distributeType == 3">
				...
				<el-divider
					><span style="font-size: 17px"><b>队列参数配置</b></span></el-divider
				>
				...
				<el-form-item
					v-if="dataForm.distributeJson.distributeMq.mqType"
					label="数据推送批次大小"
					label-width="auto"
					prop="distributeJson.distributeMq.fetchSize"
				>
		...
		<el-dialog v-model="tableNameMapperDialogVisible" title="查看表名映射关系">
		...
		<el-dialog v-model="columnNameMapperDialogVisible" title="查看字段映射关系">
