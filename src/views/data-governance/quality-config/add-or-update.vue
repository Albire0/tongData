	...
	<el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false">
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="100px" @keyup.enter="submitHandle()">
			...
			<el-form-item label="检测表" prop="sourceTableMetaId" label-width="auto">
				<el-tree-select
					v-model="dataForm.sourceTableMetaId"
					:props="props"
					:data="metaTableList"
					clearable
					filterable
					@current-change="sourceTableChange"
				>
			...
			<div v-show="!!dataForm.sourceTableMetaId">
				...
				<el-button type="success" size="small" @click="addConfig">添加配置</el-button>
				<div v-for="(item, index) in dataForm.columnConfig" style="margin-top: 10px">
					...
					<el-select v-model="item.ruleId" style="margin-left: 8px" clearable filterable placeholder="规则类型">
						<el-option v-for="(rule, index) in qualityRuleList" :key="rule.id" :label="`${rule.name}`" :value="rule.id"></el-option>
					</el-select>
					<el-date-picker
					...
					<el-input
						v-if="
							item.ruleId == 12 &&
							item.sourceColumnType != 'java.sql.Timestamp' &&
							item.sourceColumnType != 'java.sql.Time' &&
							item.sourceColumnType != 'java.sql.Date'
						"
						v-model="item.rangeStart"
						placeholder="开始值"
