	...
	<el-card body-style="height: calc(100vh - 170px )" @click="OptionCardClose($event)">
		<div>
			<SideLayout class="fp-content" left-title="主数据目录" right-title="主数据模型">
				...
				<template #right>
					<div v-show="infoView" v-loading="detailLoading" class="master-data-right">
						...
						<div class="dsr-part">
							<div class="info-part">
								<h6>表信息</h6>
								<el-form ref="tableInfoRef" :model="tableInfo" label-width="100px" :rules="tableInfoRules">
									<el-row>
									...
									<el-row>
										...
										<el-col :span="7">
											<el-form-item label="备注" prop="description">
												<el-input v-if="!isDetail" v-model="tableInfo.description" type="textarea" :rows="3" placeholder="备注"></el-input>
								...
								<el-table :data="columns" style="width: '100%'" :height="isDetail ? 'calc(100vh - 580px )' : 'calc(100vh - 605px )'" size="small">
									<el-table-column align="center" show-overflow-tooltip props="name" label="字段名称" width="200">
										<template #default="scope">
											<el-input v-if="!isDetail" v-model="scope.row.name" placeholder="字段名称" />
									...
									<el-table-column align="center" label="数据类型" width="200">
										<template #default="scope">
											<fast-select
												...
												dict-type="field_type"
												placeholder="请选择"
												@change="dataTypeChagne(scope.row)"
											></fast-select>
											...
											<div v-if="!isDetail">
												<el-input
													v-if="
														...
														scope.row.fieldType == 7 ||
														scope.row.fieldType == 8 ||
														scope.row.fieldType == 9 ||
														scope.row.fieldType == 10 ||
														scope.row.fieldType == 11
													"
													disabled
												></el-input>
					...
					<el-empty v-if="currentNode.type != 1" description="请选择左侧目录 获取详情" />
				</template>
			</SideLayout>
		</div>
		<!-- 右键菜单 -->
		...
		>
			<el-button v-show="ckRightOptionData.optionData.type == 0" class="option-card-button" @click="appendChild(0)">添加子目录</el-button>
			<el-button v-show="ckRightOptionData.optionData.type == 0" class="option-card-button" @click="appendChild(1)">添加主数据</el-button>
		...
		>
			<div v-loading="pickColunmsLoading" style="height: 400px; overflow-y: auto; padding: 10px">
				...
				<el-table
					:data="pickTableColumns"
					style="width: 100%; margin-top: 10px"
					size="small"
					height="200px"
					@selection-change="pickColumnsSelectChange"
				>
...
import {
	saveMasterModelApi,
	updateMasterModelApi,
	getMasterModelApi,
	deleteMasterModelApi,
	releaseMasterModelApi,
	cancelReleaseMasterModelApi
} from '@/api/data-governance/masterModel'
 ...
 * 添加目录树子菜单
 */
const appendChild = (type: number) => {
	let nodeData = ckRightOptionData.optionData
	addOrUpdateRef.value.init(null, nodeData.id, nodeData.path, 0, type)
	OptionCardClose()
}
/**
 * 重命名
