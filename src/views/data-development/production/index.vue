<template>
	<el-card>
		<div class="headerBox" @click="OptionCardClose($event)">
			...
			<div v-show="editableTabsValue != '0'" calss="headRight">
				...
				<el-tooltip class="box-item" effect="dark" content="保存 (ctrl+s) " placement="top-end">
					<el-button text @click="saveTask()"><img src="@/assets/save.png" /></el-button>
				</el-tooltip>
				<el-tooltip class="box-item" effect="dark" content="检查sql有效性" placement="top-end">
					<el-button v-show="currentTaskType != '3'" text @click="explainSql()"><img src="@/assets/check.png" /> </el-button>
				</el-tooltip>
				<el-tooltip class="box-item" effect="dark" content="同步执行当前作业" placement="top-end">
					<el-button v-show="currentTaskType == '1' || currentTaskType == '2'" :loading="executeSqlButton" text @click="executeSql()"
				...
				<el-tooltip class="box-item" effect="dark" content="异步提交作业到集群" placement="top-end">
					<el-button
						v-show="currentTaskType == '2' || currentTaskType == '3'"
						:loading="submitTaskToClusterButton"
						text
						@click="submitTaskToCluster()"
						><img src="@/assets/submit.png" />
					</el-button>
				</el-tooltip>
				<el-tooltip class="box-item" effect="dark" content="清空日志" placement="top-end">
					<el-button text @click="clearLog"><img src="@/assets/clear.png" /></el-button>
				</el-tooltip>
			</div>
		</div>
		<div class="box" @click="OptionCardClose($event)">
			<div class="mainBox">
				<div class="left">
					<el-scrollbar>
						<el-tabs v-model="activeCatalogueName" class="demo-tabs">
                 ...
                 <el-button size="small" @click="addTab(editableTabsValue)">
                       add tab
								...
								<br /><br />
								<el-button type="primary" @click="appendCatalogueRoot">添加根目录</el-button>
								<br /><br />
								<el-tree
				...
				<!-- 中间 -->
				<div class="midBox">
					<el-tabs v-model="editableTabsValue" class="mid-tabs" type="card" closable @tab-remove="removeTab" @tab-click="clickTab">
					...
					<el-scrollbar v-show="editableTabsValue == '0'">
						<div class="summaryInfo" style="margin: 30px">
							<el-descriptions title="企通数据开发计算平台" :column="1" size="large" direction="horizontal">
								<el-descriptions-item label="概述"></el-descriptions-item>
								<el-descriptions-item label="">企通数据开发计算平台，支持 Flink 实时开发，SQL 离线开发 </el-descriptions-item>
				...
				<div class="right">
					<el-scrollbar>
						<div v-show="editableTabsValue == '0'" class="summaryInfo" style="margin: 30px">
							...
							<!--									<el-button style="font-size: 17px" link type="primary"><a href="http://zrxlh.top" target="_blank">点击前往</a></el-button>-->
						...
						<div v-show="editableTabsValue != '0' && currentTaskType == '1'" style="padding: 30px 20px">
							<el-form ref="sqlDataFormRef" :model="sqlDataForm" :rules="sqlDataFormRules">
								...
								<el-form-item v-if="sqlDataForm.sqlDbType == '1'" label="选择" prop="databaseId">
									<el-select v-model="sqlDataForm.databaseId" clearable filterable placeholder="请选择">
										<el-option
											v-for="(item, index) in sqlDataForm.databaseList"
											:key="item.id"
						...
						>
							<el-form ref="flinkSqlDataFormRef" :model="flinkSqlDataForm" :rules="flinkSqlDataFormRules">
								...
								<el-form-item v-if="currentTaskType == '2' || currentTaskType == '3'" label="执行模式" prop="type">
									<fast-select
										v-if="currentTaskType == '2'"
										v-model="flinkSqlDataForm.type"
										dict-type="production_cluster_type"
										placeholder="请选择"
										clearable
									></fast-select>
									<fast-select
										v-else
										v-model="flinkSqlDataForm.type"
										dict-type="jar_run_type"
										placeholder="请选择"
										clearable
										@change="jarRunTypeChange"
									></fast-select>
								</el-form-item>
								<el-form-item v-if="currentTaskType == '2' || currentTaskType == '3'" label="flink版本" prop="flinkVersion">
								...
								<el-form-item
									v-if="
										(currentTaskType == '2' || currentTaskType == '3') &&
										(flinkSqlDataForm.type == '1' || flinkSqlDataForm.type == '2' || flinkSqlDataForm.type == '5')
									"
									label="Flink集群实例"
									prop="clusterId"
								>
									<el-select v-model="flinkSqlDataForm.clusterId" clearable filterable placeholder="请选择">
										<el-option
											v-for="(item, index) in flinkSqlDataForm.clusterList"
											:key="item.id"
											:label="`[${item.id}]${item.name}`"
											:value="item.id"
										></el-option>
								...
								<el-form-item
									v-if="
										(currentTaskType == '2' || currentTaskType == '3') &&
										(flinkSqlDataForm.type == '3' || flinkSqlDataForm.type == '4' || flinkSqlDataForm.type == '6')
									"
									label="Flink集群配置"
									prop="clusterConfigurationId"
								>
									<el-select v-model="flinkSqlDataForm.clusterConfigurationId" clearable filterable placeholder="请选择">
										<el-option
											v-for="(item, index) in flinkSqlDataForm.clusterConfiguratioList"
											:key="item.id"
											:label="`[${item.id}]${item.name}`"
											:value="item.id"
										></el-option>
								...
								<el-form-item v-if="currentTaskType == '3'" label="JAR" prop="jarId">
									<el-tooltip content="选择要执行的 JAR 包" placement="top">
										<el-select v-model="flinkSqlDataForm.jarId" clearable filterable placeholder="请选择">
											<el-option v-for="(item, index) in jarList" :key="item.id" :label="`[${item.id}]${item.name}`" :value="item.id"></el-option>
										</el-select>
									</el-tooltip>
								</el-form-item>
								<el-form-item v-if="currentTaskType == '2'" label="FlinkSql公共代码块" prop="envId">
									<el-tooltip
										content="选择代码块之后会前置执行代码块中的内容,主要用于初始化 ddl 操作,定义全局变量等,避免每个作业重复书写"
										placement="top"
									>
										<el-select v-model="flinkSqlDataForm.envId" clearable filterable placeholder="请选择">
											<el-option
												v-for="(item, index) in flinkSqlDataForm.envList"
												:key="item.id"
												:label="`[${item.id}]${item.name}`"
												:value="item.id"
											></el-option>
								...
								<el-form-item v-if="currentTaskType == '2' || currentTaskType == '3'" label="任务并行度" prop="parallelism">
									<el-input-number v-model="flinkSqlDataForm.parallelism" :min="1" :max="10" />
								</el-form-item>
								<el-form-item v-if="currentTaskType == '2'" label="insert语句集" prop="statementSet">
								...
								<el-form-item v-if="currentTaskType != '3'" label="全局变量" prop="fragment">
								...
								<el-form-item v-if="currentTaskType == '2'" label="批处理" prop="batchModel">
								...
								<el-form-item v-if="currentTaskType == '2' || currentTaskType == '3'" label="savepoint策略" prop="savePointStrategy">
								...
								<el-form-item v-if="flinkSqlDataForm.savePointStrategy == 3" label="savepoint地址" prop="savePointPath">
								...
								<el-divider v-if="currentTaskType == '2'">
									<el-tooltip content="执行配置仅在同步运行时有效, 提交作业无效" placement="top"
										><span style="font-size: 15px">执行配置</span></el-tooltip
									>
								</el-divider>
								<el-form-item v-if="currentTaskType == '2'" label="预览结果" prop="useResult">
								...
								<el-form-item v-if="flinkSqlDataForm.useResult" label="打印流" prop="useChangeLog">
								...
								<el-form-item v-if="flinkSqlDataForm.useResult" label="自动停止" prop="useAutoCancel">
								...
								<el-form-item v-if="flinkSqlDataForm.useResult" label="数据预览最大行数" prop="pvdataNum">
			...
			<div title="上下侧边栏" class="bottomResize"></div>
			<!-- 下方 -->
			<div class="downBox">
				<el-tabs class="buttom-tabs">
					...
					<el-tab-pane>
						...
						<el-scrollbar>
							<TaskHistory ref="taskHistoryRef" :if-child="true"></TaskHistory>
						</el-scrollbar>
					</el-tab-pane>
				</el-tabs>
			</div>
		...
		>
			<el-button v-show="ckRightOptionData.optionData.ifLeaf" class="option-card-button" @click="appendCatalogueChild"> 添加子目录 </el-button>
			<el-button v-show="ckRightOptionData.optionData.ifLeaf" class="option-card-button" @click="appendCatalogueTask"> 创建作业 </el-button>
			<el-button class="option-card-button" @click="renameCatalogue">修改</el-button>
			<el-button class="option-card-button" @click="deleteCatalogue">删除</el-button>
		</div>
		<!-- 作业目录 -->
...
import {
	...
	explainSqlApi,
	executeSqlApi,
	submitTaskToClusterApi,
	getConsoleLogApi,
	clearConsoleLogApi,
	endConsoleLogApi,
	clearConsoleLogWithOutKeyApi,
	getEnvListApi
} from '@/api/data-development/task'
...
const flinkSqlDataFormRules = ref({
	...
	flinkVersion: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
	clusterId: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
	jarId: [{ required: true, message: '必填项不能为空', trigger: 'blur' }],
	...
	for (let i = 0; i < rightResize.length; i++) {
		...
		rightResize[i].onmousedown = function (e) {
			...
			document.onmousemove = function (e) {
				...
				if (moveLen - leftResize[i].offsetWidth - leftLength <= 120) {
					moveLen = 120 + leftResize[i].offsetWidth + leftLength
				}
				if (moveLen - leftResize[i].offsetWidth - leftLength + 100 >= maxT) {
					moveLen = maxT - 100 + leftResize[i].offsetWidth + leftLength
				}
