	...
	<el-card>
		<div class="accessDivClass">
			...
			>
				...
				<el-table-column prop="taskName" label="名称" header-align="center" align="center" show-overflow-tooltip></el-table-column>
				...
				<el-table-column prop="taskType" label="类型" header-align="center" align="center" width="160">
					<template #default="scope">
						<el-tag v-show="scope.row.taskType == 1" type="success">{{
						...
						<el-tag v-show="scope.row.taskType == 3" type="warning">{{
							getDictLabel(store.appStore.dictList, 'task_type', scope.row.taskType)
						}}</el-tag>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" fixed="right" header-align="center" align="center" width="320">
					<template #default="scope">
						...
						<el-button
							...
							v-auth="'data-integrate:access:update'"
							type="primary"
							link
							@click="addOrUpdateHandle(scope.row.id)"
							>编辑</el-button
						>
						<el-button v-if="scope.row.status == '0'" v-auth="'data-integrate:access:release'" type="primary" link @click="release(scope.row.id)"
							>发布</el-button
						>
						<el-button v-if="scope.row.status == '1'" v-auth="'data-integrate:access:cancle'" type="primary" link @click="cancle(scope.row.id)"
							>取消发布</el-button
						>
						...
						<el-button
							v-if="scope.row.status == '0'"
							v-auth="'data-integrate:access:delete'"
							type="primary"
							link
...
const selfHandler = id => {
	ElMessageBox.confirm('确认执行吗，如果有正在执行的任务可能会延迟执行！', '警告', {
...
const cancle = id => {
	ElMessageBox.confirm('确认取消发布吗？', '警告', {
		confirmButtonText: '确认',
		cancelButtonText: '取消',
		type: 'warning'
	})
