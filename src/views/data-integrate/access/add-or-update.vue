<template>
	<el-drawer v-model="visible" :title="!dataForm.id ? '新增' : '修改'" size="100%">
		...
		<br />
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="auto" label-position="left">
			<!-- 基本信息配置 -->
			<div v-show="active == 1">
...
<!--				</el-form-item>-->
				...
				<el-form-item v-if="dataForm.taskType == '3'" label="cron表达式" prop="cron">
					<el-input ref="inputRef" v-model="dataForm.cron" v-click-outside="onClickOutside" placeholder="cron表达式"></el-input>
				</el-form-item>
				<el-button v-if="dataForm.taskType == '3'" type="primary" style="margin-left: 90px" @click="dialogCronVisible = true"
					>配置Cron表达式</el-button
				>
			...
			<div v-show="active == 2">
				<el-form-item label="源端数据库" prop="sourceDatabaseId" label-width="auto">
					<el-select v-model="dataForm.sourceDatabaseId" clearable filterable placeholder="请选择" @change="selectChangedSourceDatabase">
				...
				<el-form-item v-if="dataForm.sourceType == 1" label="配置方式" prop="includeOrExclude" label-width="auto">
					<el-select v-model="dataForm.includeOrExclude" placeholder="请选择配置方式" @change="configTypeChange">
				...
				<el-form-item v-if="dataForm.sourceType == 1" label="表名配置" prop="sourceSelectedTables" label-width="auto">
					<el-select v-model="dataForm.sourceSelectedTables" placeholder="请选择表名" multiple filterable clearable @change="selectedTablesChange">
						<el-option
				...
				<el-form-item v-if="dataForm.sourceType == 2" label="SQL" prop="sourceSql" label-width="auto">
					<div>
						...
						<div style="height: 150px; width: 1000px">
							<SqlStudio id="accessSourceSql" ref="sqlStudioRef"></SqlStudio>
						</div>
					</div>
				</el-form-item>
				<el-form-item v-if="dataForm.sourceType == 2" label="主键" prop="sourcePrimaryKeys" label-width="auto">
				...
				<el-form-item v-if="dataForm.sourceType == 2" label="增量字段" prop="increaseColumnName" label-width="auto">
			...
			<div v-show="active == 3">
				<el-form-item label="接入方式" label-width="auto" prop="accessMode">
					<el-select v-model="dataForm.accessMode" placeholder="请选择接入方式" @change="accessModeChange">
						<el-option label="接入到ODS层" :value="1"></el-option>
						<el-option label="自定义接入" :value="2"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item v-if="dataForm.accessMode == '2'" label="接入数据库" label-width="auto" prop="targetDatabaseId">
					<el-select v-model="dataForm.targetDatabaseId" clearable filterable placeholder="请选择" @change="selectChangedTargetDatabase">
						<el-option v-for="(item, index) in dataForm.databases" :key="item.id" :label="`[${item.id}]${item.name}`" :value="item.id"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item v-if="dataForm.sourceType == 2" label="接入表" prop="targetTable" label-width="auto">
					<el-input v-model="dataForm.targetTable" style="width: 300px" placeholder="接入表"></el-input>
				...
				<el-form-item v-if="!ifMongoDB" label="同步前是否删除目的表" label-width="auto" prop="targetDropTable">
					<el-select v-model="dataForm.targetDropTable">
						<el-option label="是" :value="true"></el-option>
						<el-option label="否" :value="false"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item v-if="!ifMongoDB" label="是否启用增量变更同步" label-width="auto" prop="targetDataSync">
				...
				<el-form-item v-if="!ifMongoDB && dataForm.targetDataSync" label="增量变更同步类型" label-width="auto" prop="changeDataSyncType">
				...
				<el-form-item v-if="!ifMongoDB" label="是否主键自动递增" label-width="auto" prop="targetAutoIncrement">
			...
			<!-- 映射转换配置 -->
			<div v-show="active == 4">
				<el-form-item v-if="dataForm.sourceType == 1" label="映射类型" prop="mapperType" label-width="auto">
					<fast-select
						v-model="dataForm.mapperType"
						dict-type="mapper_type"
						placeholder="映射类型"
						clearable
						@change="mapperTypeChange"
					></fast-select>
				...
				<div v-if="!!dataForm.mapperType || dataForm.sourceType == 2">
					<div v-if="dataForm.mapperType == 'ALL' || dataForm.sourceType == 2">
						<div v-if="dataForm.sourceType == 1" class="tip-content">
							<p>说明：</p>
							<br />
							<p>(1) 支持正则表达式匹配，也可直接填写需要映射的表名。</p>
							<br />
							<p>
								(2) 当表名映射规则为空时，若接入方式为接入到ods层，会自动给表名添加ods_前缀，否则代表目标表名与源表名的名称相同;
								不为空时，若接入方式为接入到ods层，会自动给映射的表名添加ods_前缀，否则与映射的表名一致。
							</p>
							<br />
						...
						<div v-if="dataForm.sourceType == 2" class="tip-content">
							<p>说明：</p>
							<br />
						...
						<div v-if="dataForm.sourceType == 1">
							<br />
							<el-button type="success" round @click="addTableNameMapperListRow()">添加表名映射</el-button>
							<el-button type="primary" round @click="previewTableNameMapList()">预览表名映射</el-button>
							<el-table :data="dataForm.tableNameMapper" size="small" border height="110" style="margin-top: 15px">
								<template #empty>
								...
								<el-table-column label="操作" width="220">
									<template #default="scope">
										<el-button size="small" type="danger" @click="deleteTableNameMapperListItem(scope.$index)">删除</el-button>
									</template>
								</el-table-column>
							</el-table>
						</div>
						<div>
							<br />
							<el-button type="success" round @click="addColumnNameMapperListRow()">添加字段名映射</el-button>
							<el-button type="primary" round @click="previewColumnNameMapList()">预览字段名映射</el-button>
							<el-table :data="dataForm.columnNameMapper" size="small" border height="110" style="margin-top: 15px">
								<template #empty>
								...
								<el-table-column label="操作" width="220">
									<template #default="scope">
										<el-button size="small" type="danger" @click="deleteColumnNameMapperListItem(scope.$index)">删除</el-button>
									</template>
								</el-table-column>
							</el-table>
						</div>
						...
						<div v-if="dataForm.sourceType == 1">
							<el-form-item label="表主键(唯一字段)" prop="pks" label-width="auto">
								<el-input v-model="dataForm.pks" style="width: 300px" placeholder="填写表的主键" clearable></el-input>
								<el-tooltip placement="right" content="非必填,不填程序会自行获取表主键;多个用英文逗号隔开,适用于所有表主键一致的情况">
									<el-icon><QuestionFilled /></el-icon>
								</el-tooltip>
							</el-form-item>
							<el-form-item v-if="dataForm.changeDataSyncType == 'INCREASE_COLUMN'" label="增量字段" prop="increaseColumnName" label-width="auto">
								<el-input v-model="dataForm.increaseColumnName" style="width: 300px" placeholder="填写表的增量字段" clearable></el-input>
					...
					<div v-else>
						<el-form-item label="选择表名" label-width="auto">
							<el-select v-model="mapperTableName" placeholder="请选择需要配置的表名" filterable clearable @change="mapperTableNameChange">
								<el-option v-for="(item, index) in mapperTableList" :key="index" :label="item" :value="item"></el-option>
							</el-select>
						...
						<div v-if="!!mapperTableName">
							...
							<el-table :data="configMap[mapperTableName].regexTableMapper" size="small" border height="75" style="margin-top: 15px">
								<el-table-column label="表名匹配的正则名" width="320">
									<template #default="scope">
										<el-input v-model="scope.row.fromPattern" type="string"> </el-input>
									</template>
						...
						<div v-if="!!mapperTableName">
							<br />
							<el-button type="success" round @click="addSingleColumnNameMapper()">添加字段名映射</el-button>
							<el-button type="primary" round @click="previewSingleColumnName()">预览字段名映射</el-button>
							<el-table :data="configMap[mapperTableName].regexColumnMapper" size="small" border height="110" style="margin-top: 15px">
								<template #empty>
									<span>请点击"添加字段名映射"按钮添加字段名映射关系记录</span>
								</template>
								<el-table-column label="字段名匹配的正则名" width="320">
								...
								<el-table-column label="操作" width="220">
									<template #default="scope">
										<el-button size="small" type="danger" @click="deleteSingleColumnName(scope.$index)">删除</el-button>
									</template>
								</el-table-column>
							</el-table>
						</div>
						...
						<div v-if="!!mapperTableName">
							<el-form-item label="表主键(唯一字段)" label-width="auto">
								<el-select v-model="configMap[mapperTableName].sourcePrimaryKeys" placeholder="请选择" clearable multiple>
									<el-option
										v-for="(item, index) in mapperTableColumns"
										:key="item.fieldName"
							...
							<el-form-item v-if="dataForm.changeDataSyncType == 'INCREASE_COLUMN'" label="增量字段" label-width="auto">
			...
			<div v-show="active == 5">
				<el-descriptions size="default" :column="1" label-class-name="el-descriptions-item-label-class" border>
					...
					<el-descriptions-item v-if="dataForm.sourceType == '2'" label="源端SQL">{{ dataForm.sourceSql }}</el-descriptions-item>
					<el-descriptions-item v-if="dataForm.sourceType == '1'" label="源端表选择方式">
						<span v-if="dataForm.includeOrExclude == '1'"> 包含表 </span>
						<span v-if="dataForm.includeOrExclude == '0'"> 排除表 </span>
					</el-descriptions-item>
					<el-descriptions-item v-if="dataForm.sourceType == '1'" label="源端表名列表">
						<span v-show="dataForm.includeOrExclude == '1' && (!dataForm.sourceSelectedTables || dataForm.sourceSelectedTables.length == 0)"
							><b>所有物理表</b></span
						>
						<p v-for="item in dataForm.sourceSelectedTables" :key="item">{{ item }}</p>
					</el-descriptions-item>
					<el-descriptions-item v-if="dataForm.accessMode == '2'" label="目的端数据源"
						>[{{ dataForm.targetDatabaseId }}]{{ dataForm.targetDatabase.name }}</el-descriptions-item
					>
					<el-descriptions-item v-if="dataForm.sourceType == '2'" label="目的表">{{ dataForm.targetTable }}</el-descriptions-item>
					<el-descriptions-item v-if="dataForm.sourceType == '1'" label="只创建表">{{ dataForm.targetOnlyCreate }}</el-descriptions-item>
					...
					<el-descriptions-item v-if="dataForm.targetDataSync" label="增量变更同步类型">{{
						dataForm.changeDataSyncType == 'PK_ORDER_COMPARE' ? '主键排序比对' : '增量字段'
					}}</el-descriptions-item>
					<el-descriptions-item v-if="dataForm.sourceType == '1'" label="是否创建索引">{{ dataForm.targetIndexCreate }}</el-descriptions-item>
					<el-descriptions-item v-if="dataForm.accessMode == '2'" label="是否表名字段名转小写">{{ dataForm.targetLowerCase }}</el-descriptions-item>
					<el-descriptions-item v-if="dataForm.accessMode == '2'" label="是否表名字段名转大写">{{ dataForm.targetUpperCase }}</el-descriptions-item>
		...
		<template #footer>
			<el-button v-if="active > 1" round size="large" @click="pre()"> 上一步 </el-button>
			<el-button v-if="active > 0 && active < 5" round size="large" @click="next()"> 下一步 </el-button>
			<el-button v-if="active == 5" type="primary" round size="large" @click="submitHandle()"> 提交 </el-button>
		</template>

		<el-dialog v-if="active == 4" v-model="tableNameMapperDialogVisible" title="查看表名映射关系">
		...
		<el-dialog v-if="active == 4" v-model="columnNameMapperDialogVisible" title="查看字段映射关系">
			<div v-if="dataForm.sourceType == 1 && dataForm.mapperType == 'ALL'">
				<el-select v-model="preiveTableName" clearable filterable placeholder="请选择" @change="queryPreviewColumnNameMapperList">
...
import FastProjectSelect from '@/components/fast-project-select/src/fast-project-select.vue'
...
const onClickOutside = () => {
	unref(cronPopover).popperRef?.delayHide?.()
}
